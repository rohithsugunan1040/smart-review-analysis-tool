...
from src.charts import create_pie_chart, keyword_analysis
...
def extract_product_name(url):
    ...
    # Try to find the last meaningful part of the URL
    for part in reversed(path_parts):
        if part and '-' in part:  # Most product names are hyphen-separated
            product_name = part.replace('-', ' ').strip()
            return unquote(product_name)
